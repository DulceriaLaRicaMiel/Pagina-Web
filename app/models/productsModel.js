...
class Product {
    ...
    static async create(data) {
        const { Nombre, Precio, Descripcion, STOCK, Categoria } = data; // Asegúrate de que aquí sea 'STOCK'
    ...
    static async update(IdProducto, data) {
        const { Nombre, Precio, Descripcion, STOCK, Categoria } = data;  // Aquí 'STOCK' en mayúscula
        ...
        const result = await pool.query(
            `UPDATE PRODUCTOS
             SET Nombre = $1, Precio = $2, Descripcion = $3, STOCK = $4, Categoria = $5
             WHERE IdProducto = $6 RETURNING *`,
            [Nombre, Precio, Descripcion, STOCK, Categoria, IdProducto]  // Asegúrate de usar 'STOCK'
        );
        return result.rows[0];
    }
